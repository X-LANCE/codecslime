...
class ResidualFSQ(nn.Module):
    ...
    def get_codes_from_indices(self, indices):
        ...
        all_codes = torch.stack([
            codebooks[q][indices[:, :, q]] for q in range(self.num_quantizers)
        ])
    ...
    def forward(
        ...
        # return
        aux_loss = torch.zeros(1, device = device)  # aux loss for fsq is always 0, add this for compatibility
        ret = (quantized_out, all_indices, aux_loss)
